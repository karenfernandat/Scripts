...
# Script para calcular a climatologia da temperatura sazonal para região SUDESTE
...
# Loop para calcular a média sazonal da temperatura na região Sudeste
variables = {'djf': saz.t[0]-273.15, 'mam': saz.t[2] - 273.15, 'jja': saz.t[1] - 273.15, 'son': saz.t[3] - 273.15}
...
for season, variable in variables.items():
    ...
    ax.set_title(f'TEMPERATURA - {season.upper()}', fontdict={'family': 'serif', 'weight': 'normal',
                                    'color': 'k', 'size': 14})
    plt.savefig(f'~/Imagens/temp_saz_sudeste_{season}.png')
    plt.close()
